/// @function scrLoadItems()
/// @description Loads all items into the global item list (global list of maps)
/// @return {boolean}

var _file_name = working_directory + "\\" + "itemlist" +".txt";
		...
		if (string_pos("BEGIN", _line) != 0)
		{
			...
			while (true)
			{
				...
				else if (string_pos("END", _line) != 0)
				{
					...
					ds_list_add(global.item_list, _temp_map)
					break
				}
				else
				{
					show_error("Item entry with \"" + _line + "\" not properly formed.", true)
				}
			}
		}
